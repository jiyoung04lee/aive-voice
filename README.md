...
- `app/page.tsx`
  - 업로드·전사 중·완료 화면 단계 분리
  - 파일 선택과 전사 요청, 5초 간격 상태 조회
  - 첫 등장 순서 기반 화자 이름 연결
  - 연속된 동일 화자의 발화를 대화 단위로 그룹화
  - 대화 검색·하이라이트와 TXT 다운로드
  - 개인정보 표시 마스킹
  - 오디오 재생 및 현재 대화 동기화
...
- `docs/architecture.md`
  - 요청 흐름, 인증 토큰 관리와 클라이언트 파생 처리 설계
...
2. 필요한 경우 인식할 한글 키워드와 화자 이름을 입력합니다.
3. **대화록 만들기**를 실행하고 전용 로딩 화면에서 완료를 기다립니다.
4. 완료 화면에서 화자 이름, 대화 검색, 마스킹, TXT 다운로드와 오디오 재생 기능을 사용합니다.
5. 화자 이름 연결이 반대라면 **이름 순서 바꾸기**를 선택합니다.
...
RTZR 개인정보 필터는 별도 계약이 필요한 기능입니다. 이 로컬 프로토타입은 전사 완료 후 정규식 기반 표시 후처리로 전화번호, 표준 이메일과 제한된 구어체 이메일을 가립니다. 구어체 이메일은 `앳`·`골뱅이`, `닷`·`점`, `컴` 등의 일부 표현과 제한된 영문 도메인 토큰을 처리합니다.
...
> 테스트에는 직접 제작한 약 5분 분량의 2인 모의 인터뷰 음성을 사용했습니다. 가상의 인물과 연락처로 구성했으며 실제 개인정보는 포함하지 않았습니다. 음성 파일과 전체 전사 결과는 저장소에 포함하지 않았습니다.

## 결과 미리보기

### 음성 및 화자 정보 입력

![음성 업로드와 화자 정보 입력](public/images/readme-upload.png)

### 전사 진행

![음성 전사 진행 화면](public/images/readme-loading.png)

### 화자별 인터뷰 대화록

![화자별 인터뷰 대화록 결과](public/images/readme-result.png)

## Test results

- 5분 19초 음성에서 발화 31개, 대화 18개, 화자 2명으로 표시
- 연속된 동일 화자의 발화를 대화 단위로 그룹화
- 첫 등장 순서를 기준으로 화자 역할과 선택 입력 이름 자동 연결 확인
- 6개 키워드 중 3개에서 인식 개선 관찰

...
- 하나의 Next.js 경로에서 업로드, 전사 중, 완료 상태를 단계별 화면으로 분리한다.
...
sequenceDiagram
    ...
    participant P as POST Route Handler
    participant G as GET Route Handler
    participant L as lib/rtzr.ts
    participant R as RTZR API

    U->>C: 음성 파일과 선택적 키워드·화자 이름 입력
    C->>P: POST /api/transcriptions
    P->>P: 파일과 키워드 검증
    P->>L: createRtzrTranscription()
    ...
    loop transcribing 상태에서 5초 간격, 최대 30분
        C->>G: GET /api/transcriptions/[id]
        G->>L: getRtzrTranscriptionStatus()
        L->>R: 전사 상태 조회
        R-->>L: transcribing / completed / failed
        L-->>G: 정리된 상태
        G-->>C: 상태 응답
    end

    alt completed
        C->>C: 첫 등장 순서 기반 화자 역할·이름 연결
        C->>C: 연속 발화 그룹화
        C->>C: 대화록·검색·오디오 화면 표시
    else failed 또는 timeout
        C->>C: 폴링 중단 및 안전한 오류 표시
    end
```
...
원본 `utterances` 배열은 직접 변경하지 않는다. 동일 화자가 연속해서 등장하는 원본 발화를 `ConversationTurn` 파생 데이터로 묶고, 마스킹이 적용된 각 대화의 표시 문자열을 기준으로 검색한다. 검색 결과 수는 일치 문자열 횟수나 원본 발화 수가 아니라 필터를 통과한 대화 개수다.

하이라이트는 검색어의 정규식 특수문자를 escape하고 모든 일치 구간을 문자열과 `<mark>` React 노드 배열로 나눈다. `dangerouslySetInnerHTML`은 사용하지 않는다.

### TXT 다운로드

검색으로 필터링된 목록이 아니라 전체 `ConversationTurn`을 기준으로 TXT를 만든다. 연속된 동일 화자의 발화를 하나의 대화 블록으로 출력하고, 첫 등장 순서에 따라 연결된 화자 역할·이름과 시작 타임스탬프를 포함한다. 검색 결과와 관계없이 전체 대화를 저장하며 현재 마스킹 설정과 화자 이름 순서를 적용한다. UTF-8 BOM을 포함한 `Blob`과 Object URL로 다운로드한 뒤 임시 anchor와 URL을 정리한다.
...
활성 인덱스가 바뀌고 오디오가 실제 재생 중일 때만 해당 항목을 화면 상단 근처로 부드럽게 스크롤한다. 활성 상태 계산은 원본 발화 인덱스를 유지한다. 활성 발화가 포함된 대화 전체를 강조하고, 다른 대화로 이동할 때만 자동 스크롤한다. 같은 대화 내부의 발화가 바뀔 때는 반복해서 스크롤하지 않는다.
...
- 일반적인 영문 표준 이메일 주소
- `앳`·`골뱅이`, `닷`·`점`, 일부 도메인 발음과 영문 도메인 토큰으로 구성된 제한적인 구어체 이메일
